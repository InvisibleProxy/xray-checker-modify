...
func TestShortenedTransferReportsItsRateInsteadOfATransportError(t *testing.T) {
	...
	for name, line := range map[string]string{
		...
		if !strings.Contains(line, "5.79 Мбит/с") {
...
func TestShortenedTransferIsNeverHealthy(t *testing.T) {
	...
	for _, result := range []speedtest.Result{slow, fast} {
		...
		if _, _, healthy := groupSpeedResults([]speedtest.Result{result}, 100); len(healthy) != 0 {
...
func TestShortenedTransferIssueLineSpellsOutTheShortfall(t *testing.T) {
	...
	for _, want := range []string{"5.79 Мбит/с", "таймаут", "30.0 с"} {
...
func TestMergedReportKeepsTimeoutCategoriesDisjoint(t *testing.T) {
	below := shortenedTransfer()
	above := shortenedTransfer()
	above.StableID, above.Name, above.Mbps = "fast-timeout", "fast timeout", 500
	complete := speedtest.Result{StableID: "complete", Name: "complete", Mbps: 500}
	cfg := DefaultConfig()
	cfg.LowSpeedThresholdMbps = 100
	report := speedtest.RunReport{Source: "telegram", Results: []speedtest.Result{
		below, above, complete, {StableID: "failed", Name: "failed", Error: "connection refused"},
	}}
	message := (&Service{}).formatSpeedReportMessage(report, cfg, 1, 2, false)
	for _, text := range []string{message.HTML, message.RichHTML} {
		for _, want := range []string{"Проверено: <b>4</b>", "В норме: <b>1</b>", "Ниже порога: <b>1</b>", "Таймаут: <b>1</b>", "Ошибки: <b>1</b>", "таймаут", "5.79 Мбит/с", "500 Мбит/с"} {
			if !strings.Contains(text, want) {
				t.Fatalf("missing %q: %s", want, text)
			}
		}
	}
	if got := formatSpeedHistoryLine(above, 100); strings.Contains(got, "порог") || strings.Contains(got, "✅") {
		t.Fatalf("timeout above the threshold must not read as a low or healthy rate: %s", got)
	}
	if !strings.Contains(message.ReplyMarkup, "node:fast-timeout") {
		t.Fatal("timeout node lost its direct action")
	}
}
