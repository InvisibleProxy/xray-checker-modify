...
Remote observation никогда не меняет Availability, статус ноды, downtime, incidents, speedtest/retry или Remnawave и не создаёт самостоятельный Telegram alert. Автоматический слой может добавить вероятностную строку к speedtest-алерту, решение об отправке которого уже принято по локальному результату, и сохранить свою пробу рядом с замером, который её вызвал. Сама session хранится только в памяти controller-а, экспортируется отдельным sanitized JSON и исчезает после restart; в истории замеров остаётся её краткая копия — вердикт, поставленная агенту задача и подписанное наблюдение. Эта копия справочная: классификацию замера, статистику и график она не меняет. Единственный multi-agent workflow — периодический sweep достижимости, описанный ниже; он тоже ничего не пишет в operational state.

Автоматика запускается по итогам замера, который завершился технической ошибкой либо показал скорость ниже действовавшего порога — независимо от того, выполнялся ли country fallback и уходит ли Telegram-алерт. Offline-нода пропускается: замер не выполнялся, воспроизводить нечего. Retry ставится в очередь до ожидания агента; одна healthy idle probe выбирается автоматически — из подключённых берётся та, чей последний контакт свежее, — а одинаковый `StableID` дедуплицируется на время cooldown. Cooldown занимает только та session, которая что-то принесла: если агент был выбран в окне устаревания liveness и так и не забрал задание, session истечёт пустой и cooldown освободится, а не заглушит ноду на полчаса. Ненадёжный observation cooldown сохраняет — он ответил и назвал агента. Вердикт «воспроизвелось» по низкой скорости выносится только когда весь интервал агентского замера ниже порога: агент отдаёт целые Mbps, и на границе округления результат считается невоспроизведённым, чтобы не поднимать ложную тревогу. В алерте называется тот агент, который подписал observation. Per-node и project maintenance, а также retired-ноды пропускаются. В `Settings → Agents` виден read-only runtime snapshot автоматики.

Пробу видно во вкладке `Speed History`: у замера появляется колонка `Agent probe` с вердиктом, а кнопка открывает окно с задачей, которую получил агент, метаданными задания и его наблюдением. `PROBE_AUTOMATION_ALERT_WAIT_SECONDS` ограничивает только ожидание алерта; история дожидается пробы дольше — до истечения самого задания — и обновляет запись, когда агент ответил.

Замер и проба не делят ноду. Прогон, стартующий по нодам, которые сейчас мерит агент, ждёт окончания пробы и только потом начинает качать: агенту передают тот же объём, что перекачал прогон, и две передачи по одному аплинку дали бы два числа, из которых ни одно не описывает ноду. Прервать пробу нельзя — агент выполняет задание до собственного дедлайна, — поэтому это именно ожидание, ограниченное сроком жизни задания (по умолчанию 5 минут). На практике оно возникает только при ручном запуске сразу вслед за неудачным прогоном: плановый интервал и 30-минутный повтор заведомо длиннее. Задержка дольше секунды пишется в лог строкой `Speed test waited … for agent probes`.
...
PROBE_AUTOMATION_COOLDOWN_MINUTES=15
...
`streak` считает **независимые** наблюдения: он растёт только когда локальная выборка обновилась с прошлого раза. Два наблюдения против одного и того же локального результата — это одно свидетельство, прочитанное дважды, а смысл `streak` именно в том, чтобы пережить устаревший локальный результат. Побочный эффект приятный: перепроверку нельзя «задолбить» до подтверждения.
...
Вкладка `Reachability` показывает находки отдельной таблицей (подтверждённые сверху) и полную матрицу с легендой; у самого checker-а в матрице своя колонка, рядом с агентами. `Sweep now` запускает внеочередной полный проход, повторный запрос во время идущего прохода игнорируется. Кнопка `Перепроверить` рядом с находкой и в строке матрицы опрашивает всех агентов по одной ноде — подтвердить или закрыть одно расхождение не стоит полного обхода. Это тот же job с тем же trigger и профилем, поэтому результат — обычное наблюдение, а не свидетельство второго сорта; отметку «последний опрос» точечная перепроверка не двигает, потому что про остальные строки она ничего не говорит.

Колонка агента, ушедшего со связи, из матрицы не пропадает: последнее, что он видел, остаётся на экране приглушённым и с меткой `⧗`, а в подсказке — время последнего наблюдения и его вердикт. Такое наблюдение считается устаревшим и в находки не попадает: оно описывает уже прошедший момент и не может ни поднять алерт, ни удержать его. Колонка исчезает только когда агент удалён или отозван.

Ручная диагностика из карточки ноды в матрицу **не** пишет, и это намеренно. Она допускает любой профиль, включая TLS и DNS, чей статус не сопоставим со статусом sweep-пробы, разрешена на нодах в maintenance, где локальный статус никто не поддерживает, и остаётся read-only действием оператора. Для перепроверки достижимости есть `Перепроверить`, который идёт по тому же пути, что и плановый проход. Ноды в maintenance пропускаются: их локальный статус никто не обновляет, сравнивать не с чем. Проект в maintenance останавливает sweep целиком. Матрица лежит в `data/reachability.json` и переживает restart; это кэш наблюдений, поэтому его потеря стоит одного интервала, а не корректности.
...
При физическом drag карточка остаётся под захваченной точкой, а скорость отпускания влияет на прогнозируемую позицию и пружинное завершение движения; незавершённое движение можно сразу перехватить или развернуть. Навигационные вкладки работают стрелками/Home/End, заголовки таблицы Nodes Overview сортируются Enter/Space, а интерфейс учитывает системные настройки reduced motion, reduced transparency, increased contrast и увеличенный размер текста. Всё это относится только к `/admin`; публичный dashboard не меняется.
...
Вкладка `Speed History` фильтруется по периоду теми же пресетами, что и графики на dashboard: 1 и 3 часа, 24 часа, 3, 7, 14 и 30 дней, `Всё` и `Даты` с произвольным диапазоном. По умолчанию открыт период 7 дней. `Всё` запрашивает историю без границ, то есть всё, что осталось в пределах `History retention days`. Произвольный диапазон использует те же правила, что и API: `from` включительно, `to` не включительно, поэтому выбор одной даты покрывает эти сутки целиком.

Для плановых работ выберите active-ноду и используйте контекстную кнопку `Maintenance`/`Resume` в `Controls → Actions`; тот же переключатель доступен в `Nodes Overview`. Режим сохраняется по `StableID` в `data/node_registry.json` и переживает restart. Maintenance-нода исключается из availability/downtime-статистики, быстрого recovery-loop, планового speedtest и Telegram alerts/retries, но медленный полный availability-обход продолжает выполнять через неё технический proxy-probe для Remnawave announce. Результат такого probe не попадает в Prometheus, node incidents, downtime и Telegram, но остаётся виден в админке: карточка ноды показывает измеренную задержку, а слово `Обслуживание` переезжает в подпись под ней, и probe попадает в график availability, чтобы линия не рвалась на время паузы. Пауза очищает сохранённый статус, поэтому до первой проверки карточка пишет `Ещё не проверялась`. Скорость при этом замирает на последнем значении: плановый speedtest paused-ноду не выбирает, обновить Mbps можно только кнопкой `Run`. Кнопки `Check` и `Run` в админке остаются доступны и не снимают maintenance; результат ручного speed-probe виден в текущем admin snapshot, но не записывается в speedtest history и не участвует в агрегатах. Telegram-команды paused-ноду пропускают. При включении закрываются текущий downtime и активный node incident; cumulative downtime, incident journal, прежняя speedtest history, mute и персональный Test URL сохраняются. `/config/<StableID>` во время обслуживания отвечает `200 Maintenance` с `X-Xray-Checker-Status: maintenance`, чтобы внешний uptime-check не записывал плановую остановку как outage. После `Resume` нода снова участвует в мониторинге, но не считается online до следующей настоящей проверки.
...
Пока проверка идёт, групповая кнопка `Check` превращается в `Cancel`: её не нужно досиживать до конца. Отмена останавливает проверку целиком, включая ту, что ещё стоит в очереди за фоновым обходом. Ноды, до которых проверка не дошла, и нода, чей запрос был оборван на середине, сохраняют прежний статус — прерванная проба не считается провалом и не открывает downtime. Фоновый обход при этом тоже прекращается и повторится по своему расписанию.
...
После добавления `REMNAWAVE_ANNOUNCE_ENABLED=true`, `REMNAWAVE_API_URL` и token перезапустите checker, откройте вкладку `Remnawave` и нажмите `Sync topology & reconcile`.

Вкладка разделена на три подвкладки: `Настройки` (policy, audience pairs и текущее состояние announce), `Конструктор сообщений` и `Локации`. Шапка со статусом интеграции, кнопки `Sync topology & reconcile` и `Сохранить настройки` остаются видимыми на любой подвкладке, поэтому сохранять изменения можно не возвращаясь в первую. Затем:
...
Для выбранной ноды в `Controls → Настройки замера для ноды` можно задать три персональных значения: Test URL, размер загрузки в МБ и порог низкой скорости в Mbps. Пустое поле означает глобальное значение, поэтому очистка возвращает ноду к общим настройкам. Размер ограничен теми же 100 МБ, что и глобальный, и не может быть меньше 64 КБ.

Персональный порог решает всё, что зависит от «низкой скорости» именно для этой ноды: переключение на country fallback, постановку 30-минутного подтверждения и то, как результат читается в отчётах и истории. Это нужно, когда ноды заведомо разного класса: на медленном тарифе 50 Mbps — норма, а на магистральном — уже деградация, и один общий порог даёт либо ложные тревоги, либо молчание.

Порог, действовавший на момент замера, сохраняется вместе с результатом. Поэтому изменение глобального порога больше не переклассифицирует прошлые замеры: отчёт, отложенное подтверждение и график истории оценивают результат тем же порогом, каким его оценил сам прогон. У записей, сделанных до появления этого поля, порог берётся текущий глобальный.
...
### Несколько подписок на dashboard

Когда источников больше одного, над блоком `Обзор` появляется ряд переключателей: `Все подписки · <имя> · <имя>` со счётчиком нод в каждом. Выбор задаёт область для всей страницы — KPI «Серверов онлайн», средняя скорость, число замеров и список нод считаются по выбранной подписке, а не по общему пулу. При одном источнике ряд не показывается.

Переключатель — это тот же фильтр `Subscription` из `Controls → Filters`, поднятый наверх: они всегда согласованы, потому что читают одно значение. Так «18 из 24 онлайн» перестаёт быть бесполезным числом, когда упал один провайдер из двух.

### Источники подписок

`SUBSCRIPTION_URL` остаётся обязательным: checker стартует именно с этих подписок, и они всегда запрашиваются с его собственным `User-Agent: Xray-Checker`. Менять это нельзя — на этот заголовок опираются Subscription Response Rules вашей панели.

Дополнительные источники добавляются в `Settings → Subscriptions` без перезапуска. У каждого свой профиль клиента:

| Профиль | User-Agent | Заголовки устройства |
| --- | --- | --- |
| `checker` | `Xray-Checker` | `X-Device-OS: CheckerOS`, фиксированный HWID |
| `happ` | `Happ/3.13.0` | `iOS` · `26.6` · `iPhone 16 Pro` · `ru-RU` · `X-Client: Happ` |
| `incy` | `INCY/2.5.5/ios CFNetwork/3860.700.1 Darwin/25.6.0` | `iOS` · `26.6` · `iPhone 16 Pro` · `ru-RU` · `X-Client: INCY` |
| `custom` | свой | свои |

Профили `happ` и `incy` шлют полный набор заголовков реального клиента: `X-Device-OS`, `X-Ver-OS`, `X-Device-Model`, `X-Device-Locale`, `X-Client`, `X-App-Version` и `X-Hwid`. Сама Remnawave читает только устройство и HWID, но панель вправе сопоставлять response rule с любым заголовком, поэтому запрос отправляется целиком, а не наполовину. HWID генерируется в том же виде, в каком его шлют iOS-клиенты — заглавный UUID, — чтобы в списке устройств панели он не выделялся среди настоящих телефонов.

Профиль нужен потому, что чужая панель отвечает по-разному в зависимости от того, кто спрашивает. Remnawave считает Happ и INCY «расширенными клиентами» и может отдать им JSON там, где другому клиенту уходит base64. А панель с включённым лимитом устройств вообще не отдаёт подписку без заголовка `x-hwid` — возвращает `404` и `x-hwid-not-supported: true`. Checker распознаёт этот ответ и пишет в ошибке, что нужен HWID, а не просто «HTTP 404».

HWID генерируется при добавлении источника и дальше не меняется: удалённая панель привязывает к нему слот устройства, и новое значение на каждый запрос отъедало бы новый слот до упора в лимит. Отдельные заголовки (`X-Ver-OS`, `X-Device-Model` и прочие) можно переопределить в блоке `Override device headers`.

**Своё имя для ноды.** Панель вправе называть outbound'ы по своей маршрутизации — `proxy-01-nl-edge-a1-nl-edge-host-01` описывает, куда идёт туннель, а не сервер. Если в записи подписки один outbound, checker берёт человеческое `remarks`; если несколько, `remarks` не может их различить, и каждая нода остаётся со своим тегом.

Выберите одну ноду и задайте имя в `Controls → Actions → Имя ноды`. Оно показывается везде: карточки, публичная страница статуса, `/config`-эндпоинты, Telegram-бот и его отчёты. Пустое поле возвращает подписочное имя. Ограничение — 64 символа.

Подписочное имя при этом никуда не девается: оно остаётся в архиве, участвует в сравнении при обновлении подписки и в определении заявленной страны, показывается в подписи карточки и продолжает находиться поиском в боте. То есть переименование меняет только то, что читают.

**Режим наблюдения.** Чужая панель редко нужна целиком, поэтому у каждого добавленного источника есть режим:

| Режим | Что делает |
| --- | --- |
| `Полный` | Измеряется, учитывается и публикуется как собственная подписка |
| `Только доступность` | Проверки и учёт остаются, плановый speedtest ноды источника не выбирает |
| `Приостановлен` | Проверка продолжается, но выводов из неё не делается: ни downtime, ни инцидентов, ни алертов, ни замеров. Ноды остаются в списке — этим режим отличается от выключения источника |

Рядом независимый переключатель `Не показывать на публичной странице статуса`: убирает ноды источника со страницы статуса, из Prometheus и из их собственных `/config`-эндпоинтов — чужая панель не тот сервис, который вы публикуете как свой. В админке такие ноды видны по-прежнему.

**В Telegram добавленных источников нет вообще.** Бот говорит только о подписке из окружения — о том сервисе, который вы держите. Ноды добавленных источников не попадают ни в список `/nodes`, ни в счётчики, ни в карточки, ни в поиск по имени, ни в алерты и отчёты о скорости. Настройки для этого нет: это правило.

**Журнал инцидентов — тоже только о своей подписке.** Правило то же и по тому же
признаку, а не по режиму: падение ноды добавленного источника видно в её
карточке и считается в её downtime, но записи во вкладке `Incidents` не заводит
ни в одном режиме. И в знаменатель массового инцидента такие ноды не входят:
иначе десяток чужих нод поднимал бы порог большинства, и авария на собственном
парке переставала бы до него дотягивать.

Одно исключение сделано намеренно: такая нода остаётся кандидатом в туннели, через которые бот дозванивается до самого Telegram при заблокированном прямом соединении. Наблюдать за чужой нодой смысла нет, а вот доставить через неё сообщение — есть.

Режим не влияет на то, что скачивается, поэтому применяется сразу, а не на следующем обновлении подписки. Env-подписки всегда наблюдаются полностью. Ручное решение по конкретной ноде — `Maintenance` и mute — сильнее режима источника.

**Источник занимает слот устройства.** Для удалённой панели checker — такой же телефон, как любой другой: он регистрируется под своим HWID и отображается в списке устройств (`iPhone 14 Pro Max`, `iOS`). На подписке с лимитом в одно устройство этот слот единственный, и checker займёт место вашего собственного клиента. Чтобы не расходовать лишний слот, впишите в поле HWID значение уже зарегистрированного устройства — панель обновит его запись вместо создания новой.

Если лимит всё же исчерпан, панель возвращает не ошибку, а подписку-заглушку: несколько «нод» с текстом вроде «Лимит устройств исчерпан» на адресе из документационного диапазона (`203.0.113.0/24`). Checker распознаёт такие записи по адресу — RFC 5737 и RFC 3849 для IP, RFC 2606 для доменов — и отбрасывает их, а если ничего кроме них не пришло, сообщает, что подписка вернула только заглушки. Иначе фиктивные ноды попали бы в мониторинг и навсегда остались бы offline.

Изменения применяются на следующем обновлении подписки — ручном или плановом, — потому что смена состава нод проходит через обычный guard подозрительной разницы.

URL подписок нигде не показываются целиком: в API и в панели остаются схема, хост и края пути (`https://panel.example/abc…xyz`). Подписочный URL — это и есть токен доступа, поэтому он не попадает ни в API, ни в логи, ни в backup. При редактировании источника поле URL можно оставить пустым — сохранится прежний.

Файл `data/subscription_sources.json` намеренно **не** входит в резервную копию по той же причине.

### Telegram

Бот показывает сначала короткую сводку и проблемы, а технические поля и успешные результаты убирает в раскрываемые детали. Интерактивные экраны «Проблемы» и «Замеры», включая кнопки перехода к истории, используют только active-ноды текущей подписки; сохранённые результаты retired-нод остаются доступны в админской истории и архиве, но в Telegram не выводятся. На Telegram Bot API с поддержкой Rich Messages используются нативные заголовки, таблицы, списки и `Details`. Для старого или локального Bot API автоматически применяется компактный HTML-вариант; при неоднозначной сетевой ошибке бот не отправляет fallback повторно, чтобы не задвоить сообщение. Повторное открытие неизменившегося экрана не заменяет Rich-сообщение компактным: ответ `message is not modified` считается успехом, а не поводом переотправить содержимое.

Экран «Замеры» показывает все ноды одним сообщением, сгруппированные по состоянию: сначала `Ошибки`, затем `Низкая скорость`, затем свёрнутый блок `В норме` с их количеством. Внутри группы порядок по времени замера. Клавиатура повторяет тот же порядок — проблемные ноды идут первыми и помечены `❌`/`⚠️`, у остальных значка нет — и разбивается на страницы только когда нод больше 24.

Список нод разбит на страницы по 8 кнопок со стрелками `‹`/`›` и счётчиком. Список команд публикуется через `setMyCommands`, поэтому клиент подсказывает их при вводе `/`.

Под каждым алертом о недоступности и восстановлении есть кнопки `Проверить`, `Открыть` и `🔕 Заглушить`. Экран `🔕 Уведомления` в карточке ноды доступен администратору и позволяет заглушить ноду на 1 или 8 часов, отдельно только алерты или только замеры, либо навсегда. Временная тишина хранится в `data/node_alert_state.json` с точным сроком, переживает restart, сама истекает и удаляется вместе с исчезнувшей нодой. Вариант «Навсегда» записывается в обычные списки mute в `data/telegram_config.json`, поэтому виден и снимается в админке. Кнопка `🔔 Включить уведомления` снимает обе разновидности сразу.

Speed-test из команды или кнопки сначала выполняет availability-check; бот подтверждает нажатие сразу и обновляет то же сообщение, а не блокирует обработку остальных команд на время проверки.

Команда `/id` отвечает и вне разрешённого чата — иначе не узнать ID для настройки, — но ограничена по частоте: не чаще одного ответа в 30 секунд одному пользователю и не чаще одного ответа в 3 секунды суммарно.
...
Первый обычный фоновый результат ниже порога (расписание или админ-панель) сначала проверяется через country fallback. Если резервный результат нормальный, событие закрывается без сообщения и отложенного теста. Только если резерв тоже медленный либо доступные резервы завершаются ошибкой, checker ставит отдельный speedtest просевших нод через 30 минут: нормальный повтор снимает событие без сообщения, а повторно низкая скорость, offline или ошибка дают алерт «проблема подтверждена». Неразрешённый `context deadline exceeded` в основной ошибке или `PrimaryError` проблемного резервного результата входит в ту же 30-минутную очередь и до подтверждения не уведомляет; здоровый fallback такой retry не создаёт. Объединена только очередь планирования: deadline остаётся технической ошибкой и учитывается как `failed`, а не `slow`. Если до due time ручной тест из админки получает по ноде финальный результат без ошибки и не ниже порога, checker отменяет общий pending retry. Результат планового теста чужой pending retry не снимает. Если агент воспроизвёл проблему со своей точки, эта пауза не нужна: две независимые точки уже увидели одно и то же, и нода попадает в текущий отчёт сразу, с агентским свидетельством в тексте. Retry при этом остаётся запланированным — он отвечает на другой вопрос, сохранится ли проблема, — и через 30 минут пришлёт «проблема подтверждена», если она никуда не делась. Любой другой вердикт агента паузу сохраняет: «не воспроизвелось» означает исправность пути агента, а не пути checker-а. Pending retry сохраняется в `node_alert_state.json` с нейтральным типом `speed-confirmation`, due time, TestConfig и списком нод с причинами, переживает restart и дедуплицируется по паре «нода + причина»: нода, ждущая подтверждения просадки, откроет отдельное ожидание, если следующий прогон упадёт с ошибкой. Снятие идёт по ноде целиком — восстановившейся ноде нечего подтверждать ни по одной причине. После завершения confirmation-run pending снимается со всех запрошенных нод, в том числе если нода успела перейти в `proxy_failure`/`offline` и была пропущена без speed-result. Старые записи без типа, `low-speed` и `deadline` автоматически мигрируют; для прежнего `deadline` срок переносится на 30 минут от исходного события. Для запуска из Telegram эти правила не задерживают запрошенный пользователем результат.
...
| `data/speedtest_results.json` | результаты и история speedtest, включая автоматическую пробу агента у замеров, которые её вызвали |
...
| `data/node_alert_state.json` | состояние Telegram-уведомлений, причины, pending 30-минутные speedtest confirmation retries и временная тишина нод |
