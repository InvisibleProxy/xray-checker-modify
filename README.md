...
Таймаут загрузки сохраняет измеренную скорость и пометку незавершённой передачи. В Telegram замер ниже порога учитывается только в «Ниже порога»; таймаут при скорости не ниже порога — отдельно в «Таймаут». В обоих случаях результат требует внимания и не входит в «В норме».

Раздел «Проблемы» показывает неисправные ноды даже при отключённых уведомлениях:
пометка 🔕 означает тишину, а не восстановление. Главное меню отдельно обозначает
паузу уведомлений на время обслуживания проекта.

В отчётах скорости категории «В норме», «Ниже порога» и «Ошибки» не пересекаются.
У медленной ноды указан порог именно её замера, результат агента виден рядом.
Если часть результатов ждёт подтверждения или исключена настройками уведомлений,
отчёт явно обозначает неполную выборку. Лимит применяется к целым блокам нод;
Rich и обычный HTML показывают одинаковое число скрытых нод.
Кнопки «Открыть» и «История» ведут к указанным в отчёте нодам, «Все замеры» —
к остальным результатам. Время отображается без секунд, с числовым часовым поясом.
...
Автоматика запускается по итогам замера, который завершился технической ошибкой либо показал скорость ниже действовавшего порога — независимо от того, выполнялся ли country fallback и уходит ли Telegram-алерт. Offline-нода пропускается: замер не выполнялся, воспроизводить нечего. Retry ставится в очередь до ожидания агента; одна healthy idle probe выбирается автоматически — из подключённых берётся та, чей последний контакт свежее, — а одинаковый `StableID` дедуплицируется на время cooldown. Агент, стоящий на хосте самой ноды, для неё не выбирается никогда: его ожидаемый source IP сравнивается с адресом ноды из подписки, а имя вместо адреса сначала резолвится в IP. Такой агент доходит до ноды, не выходя за пределы машины, поэтому сеть хостера, маршрут и фильтрация перед нодой в его замер не попадают, и ответ почти всегда «не воспроизвелось». Если свободен только он, проба не запускается, причина пишется в алерт и историю, а старт повторяется в окне ожидания алерта, как при занятом агенте; не разрешившееся имя ноды откладывает пробу до следующего прогона. Ручная диагностика и sweep достижимости агента называют сами, и это правило к ним не применяется. Cooldown занимает только та session, которая что-то принесла: если агент был выбран в окне устаревания liveness и так и не забрал задание, session истечёт пустой и cooldown освободится, а не заглушит ноду на полчаса. Ненадёжный observation cooldown сохраняет — он ответил и назвал агента. В алерте называется тот агент, который подписал observation.

Скорость агента сравнивается со скоростью прогона, а не только с порогом, и только при одинаковом сервере замера. Агенту передают ID сервера из встроенного каталога (`diagnostics.SpeedServers()`, те же ID, что в `country-test-urls.example.yaml`), на котором мерил прогон: ID, не URL, поэтому задание по-прежнему не может превратиться в произвольный fetch. Сервер определяется по хосту test URL прогона — резервного, если прогон перешёл на резерв. Вердикты по скорости:

| Вердикт | Когда | Что значит |
| --- | --- | --- |
| `reproduced` | тот же сервер, агент тоже ниже порога и не быстрее прогона больше чем в 3 раза | просадка у самой ноды; алерт уходит сразу, без 30-минутного подтверждения |
| `path_limited` | агент быстрее прогона больше чем в 3 раза | нода отдаёт скорость, потеря на пути от checker-а до ноды; если агент тоже ниже порога, это сказано отдельно |
| `not_reproduced` | агент дал порог, прогон был ниже, но того же порядка | проблема не воспроизвелась |
| `inconclusive` | другой сервер или скорость агента на границе порога (агент отдаёт целые Mbps) | сравнивать нельзя |

Пропустить подтверждение может только `reproduced`. Агент старой сборки без capability `speed-servers-v1` сервер не получает и мерит свой URL — его ответ на просадку будет `inconclusive` или `path_limited`, но не ложным `reproduced`. Сопоставимые вердикты появляются после обновления и контроллера, и агентов; порядок обновления значения не имеет. Per-node и project maintenance, а также retired-ноды пропускаются. В `Settings → Agents` виден read-only runtime snapshot автоматики.
...
Второй триггер — `PROBE_AUTOMATION_PROXY_FAILURE_ENABLED=true`, поверх `REMOTE_DIAGNOSTICS_AUTOMATION_ENABLED=true`, без которого controller не стартует. После каждой завершённой проверки доступности нода в `proxy_failure` получает одну пробу `auto_proxy_failure` на эпизод, то есть на интервал с одним и тем же `ProxyFailureSince`. Профиль соответствует `CHECK_METHOD` (`status`, `ip` или `download`), с альтернативным endpoint-ом, как у ручной диагностики. Повтор в пределах эпизода бывает только тогда, когда проба ничего не принесла: не нашлось свободного агента, задание никто не забрал, не разрешилось имя ноды. Нода, которая вернулась в `proxy_failure` в пределах cooldown, переиспользует прежний ответ. Ноды на обслуживании, ноды приостановленных и добавленных из панели подписок пропускаются. Агент на хосте самой ноды не выбирается. Лимит `PROBE_AUTOMATION_MAX_CONCURRENT` общий со speedtest-пробами, прогон замера скорости ждёт и эту пробу. Ответ виден среди диагностических сессий в карточке ноды и строкой «Агент …» в Telegram-алерте о сбое: одиночном, групповом и массовом, с временем проверки. Отправку и время алерта он не меняет. Первый алерт уходит не раньше второй неудачной проверки, поэтому к нему проба обычно уже ответила; если нет, алерт ждёт её не дольше `PROBE_AUTOMATION_ALERT_WAIT_SECONDS`.

Третий триггер — `PROBE_AUTOMATION_OFFLINE_ENABLED=true`, по тем же правилам эпизода (`DownSince`) и тем же туннельным профилем. Нода, до которой checker не достаёт совсем, из сети с фильтрацией выглядит так же, как нода с заблокированным IP, и ответ агента разделяет два случая, требующих разных действий: «у агента нода работает → недоступна только с пути checker-а» и «не работает и у агента → хост или хостер». Если у агента хост отвечает, а туннель нет, алерт говорит, что, вероятно, упал сервис на ноде. Эпизод другого вида — `proxy_failure`, сменившийся `offline`, — получает собственную пробу даже внутри cooldown, и алерт показывает только ответ на свой вид сбоя.

Итог каждой автоматической пробы и каждая ячейка sweep-а, не совпавшая с checker-ом, пишутся в журнал `data/diagnostic_verdicts.jsonl` — с обеими сторонами сравнения: статус или скорость и сервер прогона, статус, скорость и сервер агента. Сессии живут в памяти и исчезают после restart, а журнал хранит итог `DIAGNOSTIC_VERDICT_RETENTION_DAYS` дней (90 по умолчанию). Он справочный: operational код его не читает, в backup он не входит. Журнал виден на вкладке `Reachability` (таблица `Agent verdicts` с фильтром по источнику) и доступен через `GET /api/v1/admin/diagnostic-verdicts`.
...
PROBE_AUTOMATION_PROXY_FAILURE_ENABLED=true
PROBE_AUTOMATION_OFFLINE_ENABLED=true
DIAGNOSTIC_VERDICT_RETENTION_DAYS=90
...
Вкладка `Reachability` показывает находки отдельной таблицей (подтверждённые сверху) и полную матрицу с легендой; у самого checker-а в матрице своя колонка, рядом с агентами. Когда нод несколько подписок, рядом с кнопками появляется фильтр `Subscription`: он сужает и находки, и матрицу до одной подписки и запоминается в браузере. Обход при этом не меняется — агентов по-прежнему спрашивают про все ноды, фильтр только про то, что вы читаете. При одной подписке фильтр не показывается. `Sweep now` запускает внеочередной полный проход, повторный запрос во время идущего прохода игнорируется. Кнопка `Перепроверить` рядом с находкой и в строке матрицы опрашивает всех агентов по одной ноде — подтвердить или закрыть одно расхождение не стоит полного обхода. Это тот же job с тем же trigger и профилем, поэтому результат — обычное наблюдение, а не свидетельство второго сорта; отметку «последний опрос» точечная перепроверка не двигает, потому что про остальные строки она ничего не говорит.
...
В правом нижнем углу раскрытой карточки переключатель `Speedtest`/`Availability` меняет dataset этого же графика. Блок статистики резервирует одинаковую высоту в обоих режимах, поэтому перенос длинной строки `Checks` не сдвигает область графика при сравнении. Availability-режим показывает latency реальных proxy-checks в ms, статистику `online`/`proxy_failure`/`offline`, crosshair и tooltip на тех же периодах; история хранится по `StableID` за тот же срок, что и speedtest. Процент в строке `Checks` — uptime, доля проверенного времени online, а не доля успешных проверок: recovery-loop перепроверяет недоступную ноду каждые `PROXY_RECOVERY_INTERVAL` секунд, и при счёте по проверкам час простоя весил больше суток работы. Каждая проверка покрывает время до следующей, но не дольше 2,5 × `PROXY_CHECK_INTERVAL`; процент округляется вниз, так что любой простой оставляет его ниже 100%. Пока админка не получила интервал из `/api/v1/config`, процент не показывается: оценка по интервалам между самими проверками вернула бы тот же перекос. Линия рвётся, когда соседние online-проверки разделяет больше тех же 2,5 × `PROXY_CHECK_INTERVAL`: админка берёт интервал из `/api/v1/config`, а без него — медиану интервалов между проверками окна. Пунктир соединяет участки только через время, когда проверок не было; неудачные проверки он не пересекает, их показывает полоса у нижней границы. Если online-замеров не меньше 20, верхняя граница графика адаптируется по P90, чтобы редкие TTFB-пики не сплющивали основной диапазон. Пики выше шкалы отмечаются жёлтым маркером у верхней границы; реальное значение остаётся в tooltip и `Maximum`. Для короткой истории показывается полный диапазон. Maintenance и probe-only проверки записываются с пометкой, чтобы линия не рвалась на время паузы, но в uptime не входят; при node merge retained history переносится в active target.
...
`Refresh Geo` в Nodes Overview проверяет только active-ноды из текущего фильтра. Retired-записи сохраняют уже накопленные GeoIP-данные, но новые внешние запросы для них не выполняются. Ссылка `↗` рядом с адресом открывает ipinfo.io для IP ноды. Если подписка публикует hostname, ссылка ведёт на адрес, в который его разрешил последний успешный `Refresh Geo`, и до первого такого обновления не показывается.
...
**Announce знает только вашу подписку.** В локациях, в подтверждениях падений и в
списке нод, который вкладка `Remnawave` предлагает для участника локации,
участвуют ноды подписки из окружения. Источник, добавленный из панели, — чужой
сервис: его падение не должно попадать в текст, который читают ваши подписчики,
поэтому оно не делает локацию частично недоступной и не поднимает announce.
Настройки для этого нет, как и в Telegram: это правило.
...
- `nodes:list` — только для телеметрии нод: строки `Панель` в Telegram и `Panel` в карточках нод админки (см. раздел Telegram); без него announce работает как прежде.

Scope `external-squads:update` разрешает PATCH всего External Squad endpoint, а не только одного header, поэтому токен всё равно считается привилегированным. `nodes:list` — только чтение списка нод. Храните его только в `REMNAWAVE_API_TOKEN`, ограничьте сетевой доступ checker → panel и не используйте административный JWT.
...
Для Remnawave `proxy_failure` и подтверждённый `offline` означают недоступность сервиса и участвуют в тех же `down`/`partial` announce-сценариях. Таймер announce считает их одним сбоем: от первой неуспешной проверки после последнего `online`. Переход между `proxy_failure` и `offline` его не перезапускает — одного потерянного ping хватает, чтобы нода на одну проверку стала `offline`, и раньше каждый такой переход откладывал сообщение ещё на 15 минут. Monitoring downtime (`DownSince`), время proxy failure и Telegram-алерты считаются по-прежнему отдельно.
...
Строку статуса checker снимает и тогда, когда сказать нечего: авария закончилась, а другая локация только начала сбоить и ещё не подтверждена, или автоматический announce выключен. Восстановленный текст он запоминает в `data/remnawave_announce_state.json`. Пока в панели стоит ровно этот текст, следующий статус дописывается после него — и многострочная база тоже, хотя по правилам для чужого текста её взять нельзя. Без этой памяти checker отказывался от текста, который вернул сам, и сквад оставался в конфликте до ручного `Adopt`. Изменённый в панели текст память теряет и дальше проходит обычные правила.
...
При частичном отказе той же location последняя строка вместо этого будет начинаться с `⚠️ Часть серверов локации «Германия»`. Remnawave обрабатывает весь текст после единственного префикса, подставляет template variables и кодирует многострочное значение в `base64:...`, поэтому клиент показывает статус с новой строки. Checker сначала читает полный `responseHeadersAdd`, меняет только регистронезависимый ключ `announce` и PATCH-ит объединённую карту. Многострочный операторский `announce` можно передать под управление явным действием `Adopt current announce as base` в карточке сквада: checker запоминает точный текст, дописывает статус после него и при следующих проходах заменяет всё, что стоит за базой. Если checker уже управляет сквадом, в панели после операторского текста стоит его строка статуса; базой тогда становится текст до неё, а не всё значение. Иначе строка статуса попала бы в базу, и после первого же снятия статуса текст в панели перестал бы с базой совпадать. `announce`, который checker создал целиком, принять как базу нельзя: операторского текста в нём нет. Захват сделан отдельной кнопкой намеренно — базу, вычисленную на лету, пришлось бы угадывать отбрасыванием знакомой строки статуса, и после потери ownership-файла старый статус стал бы частью базы, а сообщение росло бы с каждым проходом. Редактирование текста в панели перестаёт совпадать с принятой базой и снова даёт конфликт, а не тихий дрейф. Произвольный многострочный чужой `announce`, suffix которого не совпадает с известным checker-сценарием, не принимается под управление и остаётся нетронутым. Если оператор изменил либо удалил составное управляемое значение вручную, checker отказывается от ownership и показывает конфликт в админке.

Конфликт означает, что подписчики сквада не получают никакого статуса, поэтому в админке он не остаётся единственным следом. При появлении он пишется в лог с уровнем `warn`. Если checker не может писать в сквад дольше 10 минут, в Telegram-чат алертов приходит сообщение с причиной и способом исправления, а когда статус снова записан — сообщение о восстановлении. Сквад считается заблокированным и в проходы, где статус не нужен, так что пауза без попыток записи отсчёт не сбрасывает. Ручная правка управляемого значения в Telegram не уходит: checker разбирает новый текст на следующем же проходе, и если принять его не может, это становится обычным конфликтом. Состояние не переживает restart, поэтому конфликт, который держится, после перезапуска придёт ещё раз.

Если отдельное Response Rule пользователя также переопределяет `announce`, оно имеет более высокий приоритет, чем External Squad, и сообщение этой интеграции пользователь не увидит.
...
Резервный результат отображается в админке символом `↪` и сохраняется в истории. Если его скорость не ниже порога, он исключается из автоматических Telegram speed-report и confirmation retry. Если резерв тоже медленный, обычный фоновый запуск ставит повтор исходного speedtest через 30 минут; если доступные резервы завершаются ошибкой после медленного основного результата, для того же повтора сохраняется исходный low-speed результат. Замер, который не успел скачать запрошенный объём за timeout, ошибкой не считается: перекачанные байты и посчитанная по ним скорость сохраняются, а результат помечается как оборванный дедлайном. Ниже порога он читается как низкая скорость и идёт по обычному 30-минутному повтору с этой причиной; не ниже порога — как техническая проблема с тем же повтором. Дедлайн, не отдавший ни байта, остаётся ошибкой. Прямой запуск из Telegram по-прежнему возвращает результат в исходный чат и topic.
...
#### Качество пути по часам

Панель `Path quality by hour` во вкладке `Speed History` раскладывает ту же историю по часам суток: строка — нода, 24 ячейки — доля плохих замеров в этот час (ошибка, недоступность, таймаут или скорость ниже действовавшего порога), справа — доля плохих в вечерний пик и вне его, медианы и вердикты агентов по этим замерам. Нода, которая проседает каждый вечер, — это свойство маршрута до её хостера, а не цепочка инцидентов, и на оси времени она выглядит как разрозненные провалы. Повторные замеры для подтверждения не учитываются: они идут только после плохого замера и удваивали бы каждый плохой час.

Пик задаётся в часовом поясе клиентов, а не оператора: `PATH_QUALITY_PEAK_HOURS` (по умолчанию `18-24`, конец не включается, `24` — полночь) и `PATH_QUALITY_PEAK_TIMEZONE` (по умолчанию `Europe/Moscow`). Часы в панели показываются в том же поясе. Период — 7, 14 или 30 дней, фильтр подписки общий со всей вкладкой. Отчёт считается на лету из сохранённой истории (`GET /api/v1/admin/path-quality`) и ничего не сохраняет.

Тот же отчёт по собственной подписке каждый понедельник в 10:00 по часовому поясу бота уходит в Telegram — см. раздел Telegram.
...
Часовой пояс сообщений выбирается в `Settings → Telegram → Часовой пояс`. В нём печатается любое время, которое читает оператор: момент замера в отчётах, «недоступна с» в алертах, срок временной тишины. Список поясов админка берёт из базы браузера, значение хранится как имя IANA в `data/telegram_config.json`, а `Время сервера` оставляет пояс процесса — в контейнере это UTC, пока ему не задан `TZ`. Имя, которое хост не может загрузить, не сохраняется: API отвечает `400`, потому что молчаливый откат к UTC выглядел бы как сломанная настройка. База часовых поясов встроена в бинарник, поэтому выбор работает и там, где в системе нет `tzdata`.
...
Первый обычный фоновый результат ниже порога (расписание или админ-панель) сначала проверяется через country fallback. Если резервный результат нормальный, событие закрывается без сообщения и отложенного теста. Только если резерв тоже медленный либо доступные резервы завершаются ошибкой, checker ставит отдельный speedtest просевших нод через 30 минут: нормальный повтор снимает событие без сообщения, а повторно низкая скорость, offline или ошибка дают алерт «проблема подтверждена». Замер, оборванный дедлайном, входит в ту же 30-минутную очередь и до подтверждения не уведомляет; здоровый fallback такой retry не создаёт. Причина ожидания берётся по измеренной скорости: ниже порога — низкая скорость, иначе техническая проблема. В счётчиках отчёта такой замер учитывается как `slow`, а не `failed`, потому что скорость у него есть. История, записанная до этого разделения, опознаётся по прежнему тексту `context deadline exceeded` и остаётся технической ошибкой. Если до due time ручной тест из админки получает по ноде финальный результат без ошибки и не ниже порога, checker отменяет общий pending retry. Результат планового теста чужой pending retry не снимает. Если агент воспроизвёл проблему со своей точки — на том же сервере замера и со скоростью того же порядка, — эта пауза не нужна: две независимые точки уже увидели одно и то же, и нода попадает в текущий отчёт сразу, с агентским свидетельством в тексте. Retry при этом остаётся запланированным — он отвечает на другой вопрос, сохранится ли проблема, — и через 30 минут пришлёт «проблема подтверждена», если она никуда не делась. Любой другой вердикт агента паузу сохраняет: «не воспроизвелось» и «путь checker-а» означают исправность пути агента, а не пути checker-а, а «несопоставимо» не говорит ничего. Pending retry сохраняется в `node_alert_state.json` с нейтральным типом `speed-confirmation`, due time, TestConfig и списком нод с причинами, переживает restart и дедуплицируется по паре «нода + причина»: нода, ждущая подтверждения просадки, откроет отдельное ожидание, если следующий прогон упадёт с ошибкой. Снятие идёт по ноде целиком — восстановившейся ноде нечего подтверждать ни по одной причине. После завершения confirmation-run pending снимается со всех запрошенных нод, в том числе если нода успела перейти в `proxy_failure`/`offline` и была пропущена без speed-result. Старые записи без типа, `low-speed` и `deadline` автоматически мигрируют; для прежнего `deadline` срок переносится на 30 минут от исходного события. Для запуска из Telegram эти правила не задерживают запрошенный пользователем результат.
...
Если настроена интеграция с Remnawave (`REMNAWAVE_API_URL` и `REMNAWAVE_API_TOKEN`) и токену выдано право `nodes:list`, под алертом о сбое, под проблемными нодами отчёта о скорости и в карточке ноды бота появляется строка `Панель`: видит ли панель ноду (и её сообщение, если нет), сколько клиентов на ней сейчас и сколько было до начала сбоя, память, load, трафик интерфейса и uptime xray. Те же факты в том же порядке показывает строка `Panel` в раскрытой карточке ноды админки — меняется только язык подписей: что показывать и как округлять, решает один `paneltelemetry.View`. «До начала сбоя» везде означает одно и то же — начало непрерывного сбоя ноды, первую неуспешную проверку после последнего `online`, а не момент, когда потерянный ping переклассифицировал `proxy_failure` в `offline` или обратно. Падение онлайна с началом сбоя — признак, что сбой видят и клиенты, независимо от того, откуда мерит checker; память и load сразу показывают известный сценарий деградации ноды. Нода сопоставляется по адресу из подписки — адресу ноды в панели или одному из её IP. Список нод читается раз в `REMNAWAVE_NODE_TELEMETRY_INTERVAL_SECONDS` (60 по умолчанию), интеграция announce для этого не нужна; `REMNAWAVE_NODE_TELEMETRY_ENABLED=false` её выключает. Без права `nodes:list` checker один раз пишет в лог, что токену не хватает права, и повторяет попытку раз в 30 минут; состояние видно в `GET /api/v1/admin/remnawave/telemetry` и строкой `Node telemetry` во вкладке `Remnawave` админки. Если перед панелью стоит reverse proxy со списком разрешённых путей (например `caddy-with-auth`), `GET /api/nodes` должен быть в нём вместе с путями announce: иначе запрос уходит в редирект, до проверки прав не доходит, и состояние показывает `redirects are disabled`. Телеметрия — такое же свидетельство, как ответ агента: отправку и время алерта она не меняет.

Каждый понедельник в 10:00 по часовому поясу бота приходит сводка качества пути за неделю по собственной подписке: ноды с худшей долей плохих замеров в вечерний пик (с медианой, долей вне пика, худшим часом и вердиктами агентов), ноды без просадок, весь парк по часам, итоги проб по доступности из журнала вердиктов и какие агенты на связи, а какие молчат и с какого дня. `/quality` присылает ту же сводку по запросу. Выключается в `Settings → Telegram → Weekly path-quality digest`. Дата последней отправки хранится в `data/telegram_digest_state.json`; если checker пропустил понедельник больше чем на сутки, прошлая неделя не досылается.
...
| `data/remnawave_announce_state.json` | runtime ownership управляемых `announce` и тексты, которые checker восстановил, сняв свою строку статуса; не входит в backup |
| `data/project_state.json` | глобальный режим обслуживания проекта и момент включения; входит в backup |
| `data/diagnostic_verdicts.jsonl` | журнал итоговых вердиктов агентов — автоматических проб и расхождений sweep-а; retention `DIAGNOSTIC_VERDICT_RETENTION_DAYS`; не входит в backup |
| `data/telegram_digest_state.json` | время последней еженедельной сводки Telegram; не входит в backup |
