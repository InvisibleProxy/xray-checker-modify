...
Списки нод и замеров разбиты на страницы по 8 кнопок со стрелками `‹`/`›` и счётчиком, поэтому доступны все ноды, а не только первые. Список команд публикуется через `setMyCommands`, поэтому клиент подсказывает их при вводе `/`.
