...
- `nodes:list` — только для телеметрии нод в алертах (см. раздел Telegram); без него announce работает как прежде.
...
Если настроена интеграция с Remnawave (`REMNAWAVE_API_URL` и `REMNAWAVE_API_TOKEN`) и токену выдано право `nodes:list`, под алертом о сбое и под проблемными нодами отчёта о скорости появляется строка `Панель`: видит ли панель ноду (и её сообщение, если нет), сколько клиентов на ней сейчас и сколько было до начала сбоя, память, load, трафик интерфейса и uptime xray. Падение онлайна с началом сбоя — признак, что сбой видят и клиенты, независимо от того, откуда мерит checker; память и load сразу показывают известный сценарий деградации ноды. Нода сопоставляется по адресу из подписки — адресу ноды в панели или одному из её IP. Список нод читается раз в `REMNAWAVE_NODE_TELEMETRY_INTERVAL_SECONDS` (60 по умолчанию), интеграция announce для этого не нужна; `REMNAWAVE_NODE_TELEMETRY_ENABLED=false` её выключает. Без права `nodes:list` checker один раз пишет в лог, что токену не хватает права, и повторяет попытку раз в 30 минут; состояние видно в `GET /api/v1/admin/remnawave/telemetry`. Телеметрия — такое же свидетельство, как ответ агента: отправку и время алерта она не меняет.
