...
После подтверждённого восстановления действует пятиминутный hysteresis. По умолчанию checker удаляет только свою строку статуса: существовавший до интеграции `announce` восстанавливается байт-в-байт, а созданный полностью checker-ом header удаляется. Опциональный neutral status вроде `Всё стабильно` заменяет сообщение об outage; при наличии исходного base он остаётся второй строкой.
