...
Успешный резерв отображается в админке символом `↪`. Такой результат сохраняется в истории, но исключается из автоматических Telegram speed-report и low-speed retry. Прямой запуск из Telegram по-прежнему возвращает результат в исходный чат и topic.
