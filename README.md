...
Рядом два независимых переключателя. `Без уведомлений в Telegram` ничего не меняет в измерениях — только в том, о чём сообщается: удобно для панели, которую вы пока присматриваете и которая не должна будить вас ночью. `Не показывать на публичной странице статуса` убирает ноды источника со страницы статуса, из Prometheus и из их собственных `/config`-эндпоинтов: чужая панель — не тот сервис, который вы публикуете как свой. В админке такие ноды видны по-прежнему.
