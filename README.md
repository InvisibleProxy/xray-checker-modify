...
Recovery-уведомление хранится как pending до подтверждённой отправки и повторяется после временной ошибки Telegram. Оно создаётся только если уведомление о недоступности этой ноды ранее действительно было отправлено. При настройках по умолчанию восстановление обнаруживается на следующей обычной проверке и сообщение уходит примерно в пределах 5 минут после фактического возвращения ноды плюс длительность самой проверки и доставки Telegram. Фактическая верхняя граница зависит от `PROXY_CHECK_INTERVAL` и Telegram-параметра `AlertCheckMinutes`.
