...
Persisted config schema v2 содержит `MessageScenarios`: включаемые rules для single-location, multiple-location, total outage и healthy state плюс partial fallback. Дефолтные тексты совпадают с прежними сообщениями; v0/v1 получает их при normalization, а legacy `NormalMessage` мигрирует в enabled healthy rule. V1 API payload без `messages` сохраняет уже настроенные outage templates и применяет `normalMessage` только к healthy rule. Templates используют только контекстно разрешённые `{location}`, `{locations}`, `{unavailable}`, `{total}`. Backend отклоняет неизвестные braces, URL, Remnawave template delimiters, line breaks и rendered output длиннее 240 runes. Для списка более трёх локаций или oversized multiple-result применяется настраиваемый fallback.
...
После online group удерживается в сообщении до непрерывного `RecoveryMinutes` (default 5). Затем target определяется healthy rule: disabled удаляет только строку статуса checker-а, enabled рендерит настраиваемый stable text. Если до outage существовал операторский base announce, он восстанавливается/сохраняется байт-в-байт; если header полностью создал checker и healthy rule disabled, он удаляется. Тексты статуса не содержат URL/диагностики.
