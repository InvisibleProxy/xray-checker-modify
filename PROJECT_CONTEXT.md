...
Disabled/hidden Hosts исключаются. Пара `MonitoringOnly` моделирует сервисный checker squad, но никогда не становится target. `NodeMappings` всегда keyed by exact `StableID`; имя ноды и Host remark display-only. Если DNS expansion породил несколько StableID одного Host, они могут ссылаться на один Host UUID. Несколько Hosts/StableID с одинаковым `GroupKey` образуют публичную redundancy group. Все подтверждённо offline дают `down`; сочетание минимум одного healthy и минимум одного подтверждённо offline member даёт `partial`; остальные сочетания с unknown/ambiguous остаются pending/ambiguous. GroupKey агрегируется только после audience filtering, поэтому одинаковый ключ в разных тарифах не переносит health между External Squads.
...
Persisted config schema v3 содержит `MessageScenarios`: включаемые rules для полной недоступности single/multiple/all locations, частичной недоступности single/multiple redundancy groups и healthy state плюс отдельные fallback полной и частичной недоступности. v0-v2 получают отсутствующие rules при normalization, legacy `NormalMessage` мигрирует в enabled healthy rule. V1 API payload без `messages` сохраняет уже настроенные templates и применяет `normalMessage` только к healthy rule; V2 payload без partial-полей сохраняет их текущие значения. Templates используют только контекстно разрешённые `{location}`, `{locations}`, `{unavailable}`, `{affected}`, `{total}`. Backend отклоняет неизвестные braces, URL, Remnawave template delimiters, line breaks и rendered output длиннее 240 runes. Для списка более трёх локаций или oversized multiple-result применяется соответствующий настраиваемый fallback.
...
`data/remnawave_announce_state.json` schema v3 хранит точные base и составное remote value, status message, fully-down groups и partial groups. Runtime v1/v2 безопасно мигрирует: v1 остаётся ownership целого значения без base, v2 получает пустой partial-group map. Замена, восстановление или удаление разрешены только при точном совпадении remote value с ownership state; при remote mismatch текущий reconcile не выполняет PATCH, снимает ownership и публикует conflict. Сохранившееся вручную изменённое значение остаётся нетронутым. Runtime ownership и opaque base исключены из admin API и backup, чтобы не раскрывать шаблон и не импортировать право изменить header другой инсталляции. Config с policy/pairs/mappings типизирован и входит в backup.
...
Прямой результат Telegram-команды не является фоновым report: он отправляется сразу в исходные chat/topic и не фильтруется настройкой автоматических отчётов. Интерактивные экраны «Проблемы» и «Замеры» фильтруют latest speedtest results по набору текущих proxy `StableID`, поэтому retired history сохраняется в persisted state и админке, но не попадает в Telegram-текст и кнопки истории. Экран «Замеры» имеет Rich HTML-таблицу и компактный HTML fallback, как остальные основные экраны.
