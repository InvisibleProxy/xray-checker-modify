...
Admin UI показывает `Remote Diagnostics` в раскрытой карточке active-ноды: оператор выбирает подключённый agent с capability `diagnostic-v1`, запускает session, видит local snapshot, signed remote observation, TCP/ping/direct evidence и вероятностную summary, может отменить session либо скачать sanitized JSON. Controller атомарно разрешает не более одной активной session для пары `StableID` + agent; UI сохраняет выбранного агента при polling и выводит активные sessions раньше завершённых. Перед приёмом observation adapter повторно проверяет текущую generation/fingerprint; refresh делает старое задание stale. Automatic trigger, alternative endpoint и multi-agent запуск остаются следующими этапами. Существующие operational workflows не читают remote state.
