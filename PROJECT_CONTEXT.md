...
Завершённый резерв сохраняет фактический URL, исходную ошибку и метаданные endpoint в `Result` и показывается в UI символом `↪`. Только резерв со скоростью не ниже threshold получает `TelegramAlertSuppressed`; медленный резерв остаётся кандидатом на обычное 30-минутное low-speed confirmation. Если fallback был вызван медленным основным результатом, но все доступные резервы завершились ошибкой, manager возвращает исходный low-speed результат, чтобы тот не потерял 30-минутный retry. Исключение — резервный результат ниже threshold после `context deadline exceeded` основного URL: он остаётся видимым как немедленная low-speed проблема и запускает отдельный повтор исходного теста через 5 минут. Прямой Telegram-запрос остаётся обязательным ответом инициатору. Если все резервы не сработали после технической ошибки основного URL, сохраняется и уведомляется исходная ошибка.
