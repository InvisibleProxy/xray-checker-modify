...
После online group удерживается в сообщении до непрерывного `RecoveryMinutes` (default 5). Специальное recovery-сообщение не создаётся: target становится optional `NormalMessage`, а при пустом значении header удаляется. Тексты не содержат URL/диагностики и передаются Remnawave как `rwEncodeBase64:<utf8>`, чтобы subscription response получил `base64:...`.

Перед каждым write worker заново получает External Squads, case-insensitively находит `announce`, сохраняет остальные `responseHeadersAdd` и PATCH-ит объединённую карту. `data/remnawave_announce_state.json` хранит точное последнее управляемое значение и группы. Удаление/замена разрешены только при совпадении remote value с ownership state; неизвестный или вручную изменённый `announce` оставляется без изменений и публикуется как conflict. Runtime ownership исключён из backup, чтобы restore не импортировал право удалить header другой инсталляции. Config с policy/pairs/mappings типизирован и входит в backup.
