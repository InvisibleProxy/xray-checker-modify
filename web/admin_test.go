...
func TestAdminTemplateExposesRowAndGroupCheckRunActions(t *testing.T) {
	...
	for _, marker := range []string{
		...
		`xray-checker-admin-node-order`,
		`function normalizeDashboardManualOrder()`,
		...
		`function deleteDiagnosticAgent(agentId)`,
		`data-agent-delete="${escapeHtml(agent.agentId)}"`,
		`request("/diagnostic-agents/delete"`,
...
func TestWebTemplatesExposeSharedEnglishRussianLocalization(t *testing.T) {
	...
	for _, marker := range []string{
		...
		`"Check": "Check", "Run": "Run"`,
		`"Running…": "Выполняется…"`,
		`"Latency profile": "Профиль задержки"`,
		`"TLS handshake": "TLS handshake"`,
		`"DNS resolution": "Резолвинг DNS"`,
