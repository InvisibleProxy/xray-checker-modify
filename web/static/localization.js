(function () {
  ...
  const ru = {
    ...
    "the agent measured a different speed-test server": "агент мерил другой сервер замера",
    "the agent rate is too close to the threshold to tell": "скорость агента слишком близка к порогу, чтобы судить",
    "the agent was also below the threshold": "у агента тоже ниже порога",
    "Checker path": "Путь checker-а", "Inconclusive": "Несопоставимо", "Unreachable node": "Недоступная нода",
    "Server measured by the run": "Сервер замера у прогона", "Server asked of the agent": "Сервер, заданный агенту",
    "Server the agent measured": "Сервер замера у агента", "Not in the catalogue": "Нет в каталоге", "Agent's own URL": "Собственный URL агента",
    "Path quality by hour": "Качество пути по часам",
    "Share of bad measurements per node and hour of day, in the clients' time zone. The evening peak is marked on the axis; confirmation re-runs are left out. Agent verdicts show whether the node or the checker's path lost the rate.": "Доля плохих замеров по нодам и часам суток, в часовом поясе клиентов. Вечерний пик отмечен на оси; повторные замеры для подтверждения не учитываются. Вердикты агентов показывают, где терялась скорость — в ноде или на пути checker-а.",
    "Folding the speed history by hour…": "Раскладываю историю замеров по часам…", "No measurements in this period": "За этот период замеров нет",
    "bad measurements": "плохих замеров", "Median": "Медиана", "checker path": "путь checker-а", "reproduced": "воспроизвели",
    "not reproduced": "не воспроизвели", "inconclusive": "несопоставимо", "no agent": "нет агента",
    "peak": "пик", "off-peak": "вне пика", "agents": "агенты", "re-runs left out": "повторов не учтено", "hours in": "часы по",
    "Agent verdicts": "Вердикты агентов",
    "Final answers of automatic probes and reachability disagreements, newest first. Evidence only: nothing here changes a status, an incident or an alert.": "Итоговые ответы автоматических проб и расхождения опроса достижимости, новые сверху. Только свидетельства: ничто здесь не меняет статус, инцидент или алерт.",
    "All sources": "Все источники", "No verdicts recorded yet": "Вердиктов пока нет", "verdicts": "вердиктов",
    "Down for both": "Недоступна у обоих", "Agent only": "Только у агента", "Checker only": "Только у checker-а",
    "Speed test failed": "Замер не удался",
    "Weekly path-quality digest": "Еженедельная сводка качества пути",
    "Every Monday at 10:00: which nodes were worst in the evening peak, the fleet by hour and what the agents answered. /quality sends it on demand.": "Каждый понедельник в 10:00: какие ноды хуже всего вели себя в вечерний пик, весь парк по часам и что ответили агенты. /quality присылает сводку по запросу.",
    "When the unreachable-node trigger is on, a node the checker cannot reach at all gets one probe per episode as well. A node that works from the agent points at a block on the checker's path; one that fails there too points at the host or its hoster.": "Если триггер недоступной ноды включён, нода, до которой checker не достаёт совсем, тоже получает одну пробу на эпизод. Если у агента нода работает — вероятна блокировка на пути checker-а; если не работает и там — вероятен отказ хоста или хостера.",
    "A slowdown counts as reproduced only when the agent measured the same speed-test server and got a rate in the run's range. An agent many times faster than the run reads as the checker's path; another server or a rate on the threshold's edge reads as inconclusive.": "Просадка считается воспроизведённой, только если агент мерил тот же сервер замера и получил скорость того же порядка, что прогон. Агент, который в разы быстрее прогона, указывает на путь checker-а; другой сервер или скорость на границе порога — «несопоставимо».",
    "Low throughput was reproduced from another network against the same speed-test server; the node, its uplink or its hosting network is likely involved.": "Низкая скорость воспроизведена из другой сети на том же сервере замера; вероятно, дело в ноде, её канале или сети хостинга.",
    "The agent got many times the checker's rate through the same node but stayed below the threshold; most of the loss is on the path between the checker and the node, and the node is not above suspicion.": "Агент получил через ту же ноду скорость в разы выше, чем checker, но всё равно ниже порога; основная потеря — на пути от checker-а до ноды, но и нода вне подозрений не остаётся.",
    "The agent got many times the checker's rate through the same node; the loss is on the path between the checker and the node, not in the node.": "Агент получил через ту же ноду скорость в разы выше, чем checker; потеря — на пути от checker-а до ноды, а не в ноде.",
    "The agent's rate is too close to the threshold to confirm or clear the slowdown.": "Скорость агента слишком близка к порогу, чтобы подтвердить или снять просадку.",
    "The agent measured a different speed-test server, so its rate can neither confirm nor clear the slowdown.": "Агент мерил другой сервер замера, поэтому его скорость не подтверждает и не снимает просадку.",
    "The node delivered the threshold to another network; the problem is on the path between the checker and the node, or at the checker's test server.": "Нода отдала порог в другую сеть; проблема на пути от checker-а до ноды или на сервере замера checker-а.",
    "The node works from another network; it is unreachable only on the checker's path, so an IP or route block on that side is likely.": "Из другой сети нода работает; недоступна она только на пути checker-а, поэтому вероятна блокировка IP или маршрута с этой стороны.",
    "The node's tunnel works from another network against the alternative endpoint; it is unreachable only on the checker's path, so an IP or route block on that side is likely.": "Из другой сети туннель ноды работает на альтернативном endpoint; недоступна она только на пути checker-а, поэтому вероятна блокировка IP или маршрута с этой стороны.",
    "The host answers from another network but its tunnel does not carry traffic; the proxy service on the node is likely down, and the checker's path does not reach the host at all.": "Из другой сети хост отвечает, но туннель не пропускает трафик; вероятно, упал proxy-сервис на ноде, а с пути checker-а хост недоступен вовсе.",
    "The node is unreachable from another network as well; the host, its port or the hosting network is likely down.": "Нода недоступна и из другой сети; вероятно, отказали хост, его порт или сеть хостинга.",
