(function () {
  ...
  const ru = {
    ...
    "Measurement": "Замер", "Detail": "Детали", "Summary": "Сводка", "MiB": "МиБ",
    "Test URL": "Test URL", "Reserve Test URL": "Резервный Test URL", "Primary URL": "Основной URL",
    "Timed out": "Таймаут", "timed out": "таймаут",
    "No signed observation was recorded for this probe.": "Подписанный observation для этой пробы не записан.",
    "Reproduced": "Воспроизвелось", "Not reproduced": "Не воспроизвелось", "Probing": "Проба идёт", "No agent": "Нет агента",
    "Verdict": "Вердикт", "Task": "Задача", "Result": "Результат",
    "Low speed": "Низкая скорость", "Technical failure": "Технический сбой",
    "Run source": "Источник прогона",
    "Threshold": "Порог", "Measured by the run": "Измерено прогоном", "Transferred by the run": "Перекачано прогоном",
    "Probe": "Проба", "Method": "Метод", "Asked to transfer": "Просили перекачать", "Agent's own amount": "Собственный объём агента",
    "Alternative endpoint": "Альтернативный endpoint", "Job": "Задание", "Requested agents": "Запрошенные агенты",
    "Created": "Создано", "Expires": "Истекает", "Config generation": "Поколение конфига",
    "Session": "Сессия", "Session ID": "ID сессии", "Requested": "Запрошено", "Accepted": "Принято",
    ...
    "automatic diagnostic could not be started": "не удалось запустить автоматическую диагностику",
    "diagnostic session is no longer available": "диагностической сессии больше нет",
    "no signed remote observation was received": "подписанное удалённое наблюдение не получено",
    "agent direct connectivity control failed": "контроль прямой связности агента не прошёл",
    "the agent alternative tunnelled endpoint worked": "альтернативный туннельный endpoint у агента отработал",
    "agent download observation has no throughput evidence": "в наблюдении агента нет данных о скорости",
    ...
    "The agent reproduced a failure only against its download endpoint; the alternative tunnelled endpoint worked, so an endpoint-specific problem is likely.": "Агент воспроизвёл сбой только на своём download-endpoint; альтернативный туннельный endpoint отработал, поэтому вероятна проблема конкретного endpoint.",
    "The agent reached the endpoint but returned no throughput evidence; there is not enough data to compare the low-speed result.": "Агент дошёл до endpoint, но не вернул данных о скорости; сравнить результат с низкой скоростью не с чем.",
    "Low throughput was reproduced from another network; a shared node, server or upstream capacity issue is likely.": "Низкая скорость воспроизведена из другой сети; вероятна общая проблема ноды, сервера или вышестоящего канала.",
    "The speed-test problem was not reproduced from another network; the controller route or its Test URLs are more likely involved.": "Проблема speedtest не воспроизведена из другой сети; скорее задействован маршрут контроллера или его Test URL.",
    "The speed-test problem was reproduced from another network; a shared node, server or configuration issue is likely.": "Проблема speedtest воспроизведена из другой сети; вероятна общая проблема ноды, сервера или конфигурации.",
    ...
    "Not checked yet": "Ещё не проверялась", "Measured during maintenance": "Замер во время обслуживания",
    "Observation": "Наблюдение", "Observation mode": "Режим наблюдения", "Full": "Полный",
    "Availability only": "Только доступность", "Paused": "Приостановлен",
    "Unlisted": "Не публикуется",
    "Node name": "Имя ноды", "Displayed name": "Отображаемое имя ноды", "Save name": "Сохранить имя",
    "Reset": "Сбросить", "Select exactly one node": "Выберите ровно одну ноду",
    "Name from the subscription": "Имя из подписки", "From the subscription": "Из подписки",
    "Node name saved": "Имя ноды сохранено",
    "Node name reset to the subscription name": "Имя ноды сброшено на имя из подписки",
    "No successful checks in this period": "За этот период нет успешных проверок",
    "No successful measurements in this period": "За этот период нет успешных замеров",
    "Keep off the public status page": "Не показывать на публичной странице статуса",
    "Measured, counted and published like your own subscription. The bot and the incident journal stay about your own nodes.": "Измеряется, учитывается и публикуется как собственная подписка. Бот и журнал инцидентов остаются только о своих нодах.",
    "Checked and counted as usual, but scheduled speed tests skip it.": "Проверяется и учитывается как обычно, но плановый speedtest её пропускает.",
    "Nodes stay listed and probed, but nothing is counted: no downtime, no incidents, no alerts, no speed tests.": "Ноды остаются в списке и проверяются, но ничего не учитывается: ни downtime, ни инциденты, ни алерты, ни speedtest.",
    "Silence changes nothing that is measured, only what is announced. Keeping a source off the status page also removes its nodes from Prometheus and from their own /config endpoints — somebody else's panel is not the service you publish as your own.": "Тишина не меняет того, что измеряется, — только то, о чём сообщается. Скрытие источника со страницы статуса также убирает его ноды из Prometheus и из собственных /config-эндпоинтов: чужая панель — не тот сервис, который вы публикуете как свой.",
    "Stopping the speed test…": "Останавливаем speedtest…", "Speed test cancelled": "Speedtest отменён",
    "Stopping the availability check…": "Останавливаем проверку доступности…", "Availability check cancelled": "Проверка доступности отменена",
    ...
    "Geo only": "Только geo", "Geo only; the name claims no country": "Только geo; в имени страна не заявлена",
    "From geo lookup; the name claims no country": "По geo-запросу; в имени страна не заявлена",
    "Refresh Incidents": "Обновить инциденты",
    "just now": "только что",
    "Paused by the operator: not monitored and not alerted, only probed": "Приостановлены оператором: не отслеживаются и не алертят, только проверяются пробой",
    "Show nodes in maintenance": "Показать ноды на обслуживании", "Hide nodes in maintenance": "Скрыть ноды на обслуживании",
    "Hide the controls": "Скрыть панель управления",
    "Filters, schedule and settings for the selected nodes": "Фильтры, расписание и настройки выбранных нод",
    "Listed by the subscription now": "Сейчас есть в подписке",
    "Average; below it, minimum–maximum over the retained history": "Среднее; под ним — минимум–максимум за срок хранения истории",
    "For retired nodes: when the subscription last listed them": "Для выведенных нод: когда подписка перечисляла их в последний раз",
    "Node, subscription, cause": "Нода, подписка, причина",
    "No incidents match the filters": "Под фильтры не попал ни один инцидент",
    "Repeats of the same failure less than 30 minutes apart are shown as one row": "Повторы одного сбоя с разрывом меньше 30 минут показываются одной строкой",
    "One row per node, one cell per slice of the period. Paler is slower than the node's own best, yellow is below the low-speed threshold, red is a failed test.": "Одна строка на ноду, одна клетка на отрезок периода. Бледнее — медленнее лучшего результата этой ноды, жёлтая — ниже порога низкой скорости, красная — тест не прошёл.",
    "Loading every node's speed history…": "Загрузка speedtest history всех нод…",
    "No active nodes": "Нет активных нод",
    ...
    "No results for the current filters": "Под текущие фильтры результатов нет",
    ...
    "Concurrency": "Concurrency", "Timeout sec": "Таймаут, сек", "Max MiB": "Макс. МиБ",
    ...
    "Each member connects a checker StableID to the Remnawave Host used for audience filtering. A healthy member prevents a full location outage; a confirmed failed member uses the partial-availability scenario. Adding or replacing a member keeps the location and its Announce binding unchanged. Only nodes of your own subscription are listed: announce speaks to your subscribers about your service, so a source added from the panel takes no part in it and its outage cannot make a location partially unavailable.": "Каждый участник связывает StableID checker с Remnawave Host для фильтрации audience. Healthy-участник предотвращает полный outage локации, а confirmed failed member включает partial-availability scenario. Добавление или замена сервера не меняет локацию и её привязку Announce. В списке только ноды вашей подписки: announce обращается к вашим подписчикам о вашем сервисе, поэтому источник, добавленный из панели, в нём не участвует и его падение не делает локацию частично недоступной.",
    ...
    "Subscriptions": "Подписки", "Subscription sources": "Источники подписок",
    "All subscriptions": "Все подписки",
    "Add a source": "Добавить источник", "Edit source": "Изменить источник",
    "Subscription URL": "URL подписки", "Client to impersonate": "Какой клиент имитировать",
    "Override device headers": "Переопределить заголовки устройства",
    "Save source": "Сохранить источник", "Clear form": "Очистить форму",
    "from environment": "из окружения", "Read-only": "Только чтение",
    "Source": "Источник", "Client": "Клиент", "State": "Состояние",
    "Sources from the environment are listed read-only: the checker starts from them and their client identity must stay as it is. Sources added here are fetched in addition, each as the client you choose. Changes apply on the next subscription refresh.": "Источники из окружения показаны только для чтения: checker стартует с них, и их клиентская личность должна остаться прежней. Добавленные здесь источники запрашиваются дополнительно, каждый — как выбранный клиент. Изменения применятся на следующем обновлении подписки.",
    "The panel ties one device slot to this value, so it stays fixed once saved. Leave empty to generate one.": "Панель привязывает к этому значению слот устройства, поэтому после сохранения оно не меняется. Оставьте пустым, чтобы сгенерировать.",
    "A source with an impersonated client occupies a device slot on the remote panel, exactly as a phone would. On a subscription limited to one device that slot is the only one, so the checker will take the place of your own device. Reuse an existing HWID to update that device instead of claiming another.": "Источник с имитацией клиента занимает слот устройства на удалённой панели — ровно как телефон. На подписке с лимитом в одно устройство этот слот единственный, и checker займёт место вашего клиента. Впишите HWID уже зарегистрированного устройства, чтобы обновить его, а не занять ещё один слот.",
    "No subscription sources": "Источников подписок нет",
    "Subscription source saved. Refresh the subscription to apply it.": "Источник сохранён. Обновите подписку, чтобы он применился.",
    "Subscription source removed. Refresh the subscription to apply it.": "Источник удалён. Обновите подписку, чтобы это применилось.",
    "set your own": "задайте своё", "Edit": "Изменить", "Delete": "Удалить",
    "Disable": "Выключить", "Enable": "Включить", "Disabled": "Выключен",
    "Node speed-test settings": "Настройки замера для ноды",
    "Download size, MiB": "Размер загрузки, МиБ", "Low-speed threshold, Mbps": "Порог низкой скорости, Mbps",
  ...
  const patterns = [
    ...
    // The summary names only the counts that are not zero, so the maintenance
    // count is found wherever it stands rather than at a fixed position.
    [/^(\d+% proxy healthy(?: · .+?)?) · (\d+) maintenance((?: · .+)?)$/u, "$1 · $2 на обслуживании$3"],
    ...
    [/^(\d+) of (\d+) incidents · (\d+) active$/u, "Инцидентов: $1 из $2 · активных: $3"],
    [/^(\d+) incidents · (\d+) active · (\d+) rows?$/u, "Инцидентов: $1 · активных: $2 · строк: $3"],
    [/^(\d+) of (\d+) incidents · (\d+) active · (\d+) rows?$/u, "Инцидентов: $1 из $2 · активных: $3 · строк: $4"],
    [/^(\d+) min ago$/u, "$1 мин назад"],
    [/^(\d+) h ago$/u, "$1 ч назад"],
    [/^(\d+) d ago$/u, "$1 д назад"],
    [/^(\d+) total$/u, "Всего: $1"],
    [/^(<?)(\d+) min$/u, "$1$2 мин"],
    ...
    [/^(\d+) online · (\d+) proxy failure · (\d+) offline · (\d+)% uptime$/u, "$1 online · $2 proxy failure · $3 offline · $4% uptime"],
    [/^No subscription( · .+)$/u, "Без подписки$1"],
    [/^Unknown Geo data not loaded$/u, "Неизвестно; Geo-данные не загружены"],
    [/^Geo only; the name claims no country (.+)$/u, "Только geo; в имени страна не заявлена; $1"],
    [/^(.+) · geo$/u, "$1 · по geo"],
    ...
    [/^(\d+) of (\d+) proxy checks succeeded$/u, "$1 из $2 proxy checks успешны"],
    [/^Uptime counts time, not checks: a node that is down is re-checked more often\. (\d+) of (\d+) proxy checks succeeded$/u, "Uptime считается по времени, а не по числу проверок: недоступную ноду перепроверяют чаще. Успешных proxy checks: $1 из $2"],
