...
- Remote Diagnostics остаются отдельной плоскостью: `diagnostics/` не импортирует operational packages и не получает callbacks, способные менять availability/nodearchive/incidents/Telegram/speedtest/Remnawave/subscription/backup. Observation хранится только внутри ephemeral session и никогда не становится authoritative status, history, KPI либо причиной operational side effect.
