...
- `diagnostic_agents.json` содержит controller/IP-bound security state и public identities, не входит в backup/restore. Admin list никогда не возвращает token/Compose; они доступны только в непосредственном create/reissue response. Не логируйте token, private keys, proxy secret или signing payload с credential-bearing body.
