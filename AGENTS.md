...
- Remnawave write сначала перечитывает External Squads и case-insensitively merge-ит только `announce` в полной карте `responseHeadersAdd`. Существующий однострочный `rwEncodeBase64:` можно принять только как opaque base: status добавляется через `\n`, а после очистки base восстанавливается байт-в-байт. Не принимайте под управление plain/многострочное неизвестное значение и не меняйте remote value, не совпадающее с persisted ownership state. Runtime v1 мигрирует как ownership целого значения без base.
- Remnawave user messages строятся только из versioned scenario templates, а не строковых литералов в reconcile workflow. Разрешайте лишь контекстные `{location}`, `{locations}`, `{unavailable}`, `{total}`; unknown braces, URL, `{{...}}`, line breaks и output длиннее 240 runes отклоняются. Disabled scenario очищает только exact-owned suffix. Config v0/v1 получает default rules, legacy `NormalMessage` мигрирует в healthy scenario; v1 API save без `messages` не должен сбрасывать сохранённые outage templates.
