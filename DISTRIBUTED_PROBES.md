...
- наблюдение становится находкой только со второго совпадающего прохода. Локальная половина сравнения отстаёт на величину до `PROXY_CHECK_INTERVAL`, и это единственный дешёвый способ отсеять артефакт устаревания, не заставляя диагностический workflow дёргать availability loop. Счётчик считает независимые наблюдения: он растёт только когда локальная выборка обновилась, поэтому повторное чтение одного и того же локального результата ничего не подтверждает.

Точечная перепроверка одной ноды — тот же job, тот же trigger и тот же профиль, что у планового прохода; она не выполняет `Retain` и не двигает отметку последнего опроса. Ручная диагностическая сессия в матрицу не пишет вообще: она допускает любой профиль, разрешена на maintenance-нодах и остаётся read-only действием оператора.

Находка определяется по строке, а не по паре. Хранимый `Verdict` сравнивает агента с checker-ом, но checker — такая же точка наблюдения и может сам оказаться единственным отрезанным: тогда агент, который тоже не видит ноду, попадает в `agreed_down` и его сообщение теряется, хотя другой агент только что доказал, что нода жива. Поэтому при чтении матрицы из всей строки выводятся `Alive`, `Unreachable` и `LocalUnreachable`: если хотя бы одна точка достучалась до ноды, каждая недостучавшаяся — находка, включая checker. Производные поля не сохраняются, поэтому `Verdict` остаётся историческим фактом, а не переписывается задним числом.

Вердикт намеренно называет наблюдение, а не причину: `agent_only_failure` и `local_only_failure` описывают, кто с кем разошёлся, и ничего не утверждают про блокировку. Это не operational state — ни одно из состояний `global_offline`, `global_proxy_failure` или `regional_degradation` из списка ниже здесь не появляется, и ни один вердикт не читается ни availability, ни Telegram, ни Remnawave. Consensus здесь тоже нет: «нода жива» — это существование одной успешной точки, а не голосование, и одна точка наблюдения никогда не отменяет наблюдение другой.
