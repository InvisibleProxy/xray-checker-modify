...
Для отправки отчетов speed-test, оповещений о падении нод и команд управления через бота используйте раздел [Telegram-бот](/ru/integrations/telegram).
...
Эти параметры также можно настроить через приватную админ-панель `/admin`.
